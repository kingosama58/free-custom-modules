...
class ResPartner(models.Model):
    ...
    def do_partner_print(self, wizard_partner_ids, data):
        ...
        return self.env.ref(
            'at_account_followup.action_report_followup').report_action(
            self, data=datas)

    def do_partner_mail(self):
        ctx = self.env.context.copy()
        ctx['followup'] = True
        template = 'at_account_followup.email_template_at_account_followup_default'
        unknown_mails = 0
        for partner in self:
    ...
    def get_followup_table_html(self):
        ...
        if partner.unreconciled_aml_ids:
            ...
            report = self.env['report.at_account_followup.report_followup']
